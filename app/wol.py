...
def generate_modal_html(messages, title):
    ...
    return render_template(
        "generate_modal.html", title=title, message_content=message_content
    )
...
def load_computers():
    ...
    with open(computer_filename) as f:
        for line in f:
            ...
            ip_address = fields[2]
            test_type = (
                fields[3] if len(fields) >= 4 else "icmp"
            )  # Default to 'icmp' if test_type is not specified
    ...
    with open(cron_filename) as f:
        for line in f:
            if not line.startswith("#"):
                ...
                computer = next(
                    (c for c in computers if c["mac_address"] == mac_address), None
                )
                if computer:
                    computer["cron_wol_schedule"] = schedule

                # Find the computer SOL MAC address
                computer_reversed = next(
                    (c for c in computers if c["mac_address"] == reversed_mac_address),
                    None,
                )
...
def is_computer_awake_icmp(ip_address, timeout=ping_timeout):
    ...
    result = subprocess.run(
        ["fping", "-t", str(timeout), "-c", "1", ip_address],
...
def is_computer_awake_tcp(ip_address, port, timeout=tcp_timeout):
    ...
    result = subprocess.run(
        ["nc", "-z", "-w", str(timeout), ip_address, str(port)],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    return result.returncode == 0
...
def delete_computer():
    ...
    computers[:] = [
        computer for computer in computers if computer["mac_address"] != mac_address
    ]
...
def add_computer():
    ...
    if check_invalid_test_type(test_type):
        messages.append(
            f'Status check: {test_type} is invalid. Enter "icmp", "arp" or a valid TCP port number.'
        )
    if messages:
...
def edit_computer():
    ...
    if (
        ...
        and computer_to_edit["ip_address"] == ip_address
        and computer_to_edit["test_type"] == test_type
    ):
...
def add_cron(mac_address, request_cron):
    ...
    if check_invalid_cron(request_cron):
        ...
        messages.append(
            'See : <a href="https://crontab.guru/" target="_blank" rel="noopener noreferrer">Crontab maker</a>'
        )
...
def wol_or_sol_send():
    ...
    messages = []
    if is_computer_awake(ip_address, test_type):
        # 计算机在线，尝试通过 HTTP 发送关机命令
        shutdown_url = f"http://{ip_address}:8009/shutdown"
        try:
            response = requests.get(shutdown_url, timeout=5)
            if response.status_code == 200:
                messages.append(
                    f"Shutdown command successfully sent to {computer['name']} via {shutdown_url}"
                )
            else:
                messages.append(
                    f"Failed to shutdown {computer['name']}. HTTP {response.status_code}"
                )
                # 备选：发送 SoL 魔术包（可选）
                # reversed_mac = ':'.join(reversed(mac_address.split(':')))
                # send_wol_packet(reversed_mac)
                # messages.append("Falling back to Sleep-on-LAN magic packet.")
        except requests.RequestException as e:
            messages.append(f"Error connecting to {shutdown_url}: {str(e)}")
            # 可选：回退到 SoL
            # reversed_mac = ':'.join(reversed(mac_address.split(':')))
            # send_wol_packet(reversed_mac)
            # messages.append("Using Sleep-on-LAN magic packet instead.")

        title = "Shutdown"
        messages.append(
            'See: <a href="https://github.com/Misterbabou/gptwol#configure-sleep-on-lan" target="_blank" rel="noopener noreferrer">How to configure Sleep on LAN</a>'
        )
    else:
